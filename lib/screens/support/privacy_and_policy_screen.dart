        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildSectionBody(context, 'MARK collects only the information necessary for the proper functioning of the application. The application may collect:'),
              ...
              _buildBulletItem(context, 'Crash reports (if enabled in future)'),
              _buildBulletItem(context, 'Anonymous diagnostic information (if enabled)'),
              const SizedBox(height: 12),
              _buildSectionBody(context, 'MARK does not intentionally collect personal information such as:'),
              ...
              const SizedBox(height: 12),
              _buildSectionBody(context, 'unless voluntarily provided by you while contacting support.'),

              _buildSectionTitle(context, 'Location Permission'),
              _buildSectionBody(context, 'MARK may request access to your device\'s location. Location information is used only for:'),
              _buildBulletItem(context, 'Finding nearby bus stops'),
              _buildBulletItem(context, 'Displaying your current location'),
              _buildBulletItem(context, 'Planning journeys from your current position'),
              _buildBulletItem(context, 'Improving navigation experience'),
              const SizedBox(height: 12),
              _buildSectionBody(context, 'Your location is never sold or shared for advertising purposes.\n\nYou may disable location permission at any time from your device settings. Some features may not function correctly without location access.'),

              _buildSectionTitle(context, 'Internet Usage'),
              _buildSectionBody(context, 'MARK works primarily offline. Internet access may be used only for:'),
              _buildBulletItem(context, 'Checking for database updates'),
              _buildBulletItem(context, 'Downloading updated transit information'),
              _buildBulletItem(context, 'Bug reporting (future versions)'),
              _buildBulletItem(context, 'AI Assistant features (future versions)'),
              const SizedBox(height: 12),
              _buildSectionBody(context, 'Internet access is never used to collect unnecessary personal information.'),

              _buildSectionTitle(context, 'Offline Transit Database'),
              _buildSectionBody(context, 'MARK contains an offline transit database used for journey planning. The transit information stored inside the application does not contain personal user information.\n\nTransit schedules may change over time. Although every effort is made to maintain accurate information, schedule accuracy cannot be guaranteed.'),

              _buildSectionTitle(context, 'Notifications'),
              _buildSectionBody(context, 'If notification permission is granted, MARK may send notifications regarding:'),
              ...
              const SizedBox(height: 12),
              _buildSectionBody(context, 'Users can disable notifications at any time from device settings.'),

              _buildSectionTitle(context, 'Data Storage'),
              _buildSectionBody(context, 'Most application data is stored locally on your device. This may include:'),
              ...
              const SizedBox(height: 12),
              _buildSectionBody(context, 'This information remains on your device unless removed by you.'),

              _buildSectionTitle(context, 'Third-Party Services'),
              _buildSectionBody(context, 'MARK may use third-party services to improve functionality. These services may include:'),
              ...
              _buildBulletItem(context, 'Official transit data providers (future)'),
              const SizedBox(height: 12),
              _buildSectionBody(context, 'Each third-party service has its own Privacy Policy. MANOG Labs is not responsible for the privacy practices of third-party services.'),

              _buildSectionTitle(context, 'Data Security'),
              _buildSectionBody(context, 'We implement reasonable technical and organizational measures to protect your information. While we strive to protect your information, no method of electronic storage or internet transmission is completely secure. Therefore, absolute security cannot be guaranteed.'),

              _buildSectionTitle(context, 'Data Sharing'),
              _buildSectionBody(context, 'MANOG Labs does not sell, rent, or trade your personal information. Information may only be shared:'),
              _buildBulletItem(context, 'When required by applicable law'),
              _buildBulletItem(context, 'To comply with legal obligations'),
              _buildBulletItem(context, 'To protect the rights and safety of users'),
              _buildBulletItem(context, 'With your explicit consent'),

              _buildSectionTitle(context, 'User Rights'),
              _buildSectionBody(context, 'Depending on your jurisdiction, you may have the right to:'),
              _buildBulletItem(context, 'Access your information'),
              _buildBulletItem(context, 'Correct inaccurate information'),
              _buildBulletItem(context, 'Delete locally stored information'),
              _buildBulletItem(context, 'Withdraw permissions'),
              _buildBulletItem(context, 'Stop using the application at any time'),

              _buildSectionTitle(context, 'Children\'s Privacy'),
              _buildSectionBody(context, 'MARK is not specifically intended for children under the age of 13. We do not knowingly collect personal information from children. If you believe a child has provided personal information, please contact us so appropriate action can be taken.'),

              _buildSectionTitle(context, 'Transit Information Disclaimer'),
              _buildSectionBody(context, 'MARK provides journey planning assistance using available transit information. Transit schedules, routes, arrival times, and departure times are subject to change without notice.\n\nUsers are encouraged to verify important travel information whenever possible.\n\nMANOG Labs is not responsible for losses resulting from schedule changes, delays, cancellations, or operational decisions made by transport operators.'),

              _buildSectionTitle(context, 'Future Features'),
              _buildSectionBody(context, 'Future versions of MARK may introduce:'),
              ...
              const SizedBox(height: 12),
              _buildSectionBody(context, 'Whenever significant privacy-related features are introduced, this Privacy Policy will be updated accordingly.'),

              _buildSectionTitle(context, 'Changes to This Privacy Policy'),
              _buildSectionBody(context, 'MANOG Labs reserves the right to modify this Privacy Policy at any time. Changes become effective immediately after publication inside the application. Users are encouraged to review this Privacy Policy periodically.'),

              // _buildSectionTitle(context, 'Contact Us'),
              // _buildSectionBody(context, 'If you have questions, suggestions, or concerns regarding this Privacy Policy, please contact:'),
              const SizedBox(height: 12),
              ...
              //         title: Text('MANOG Labs', style: theme.textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.bold)),
              //       ),
              //       Divider(color: theme.dividerColor, height: 1),
              //       ListTile(
              //         leading: Icon(Icons.email_outlined, color: colorScheme.primary),
              //         title: const Text('support@manoglabs.com'),
              //         subtitle: const Text('Tap to copy support email'),
              //         onTap: () {
              //           // Optional Copy Implementation or URL launcher execution
              //         },
              //       ),
              //       Divider(color: theme.dividerColor, height: 1),
              //       ListTile(
              //         leading: Icon(Icons.language, color: colorScheme.primary),
              //         title: const Text('www.manoglabs.com'),
              //         subtitle: const Text('Visit our website'),
              //         onTap: () {
              //           // Optional Website URL navigation implementation
              //         },
              //       ),
              //     ],
              //   ),
