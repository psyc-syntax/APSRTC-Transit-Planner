...
class RouteResultsTripTimelineCart extends StatelessWidget{
  const RouteResultsTripTimelineCart({super.key, required this.result});
  ...
  Widget build(BuildContext context) {
    ...
    totalDistance = dateTimeData.totalDistanceCalc(result);
    totalTime = dateTimeData.totalTimeCalc(result);
          ...
          child: GestureDetector(
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) {
                    return TripNavigationScreen(results: result);
                  },
                ),
              );
            },
                ...
                child: Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        _buildParam("${max(result.path.length - 1, 1)} BUS${result.path.length > 2? "ES": ""}", Icons.directions_bus, context),
                        SizedBox(height: 4,),
